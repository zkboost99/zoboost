...
import { Search, ChevronDown, Gamepad2, Zap, Users, Crown, Sparkles, Gem, Gift, Clock, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
interface DiscordMarketplaceProps {
  products: Product[];
}

const CATEGORIES = [
  { id: 'all', label: 'All', icon: Gamepad2 },
  { id: 'discord-accounts', label: 'Accounts', icon: Users, keywords: ['account', 'acc'] },
  { id: 'discord-nitro', label: 'Nitro', icon: Crown, keywords: ['nitro', 'premium'] },
  { id: 'discord-nitro-basic', label: 'Nitro Basic', icon: Gem, keywords: ['basic'] },
  { id: 'discord-server-boosts', label: 'Server Boosts', icon: Zap, keywords: ['boost', 'server boost'] },
  { id: 'discord-members', label: 'Members', icon: Users, keywords: ['member', 'members'] },
  { id: 'discord-decorations', label: 'Decorations', icon: Sparkles, keywords: ['decor', 'decoration', 'avatar'] },
  { id: 'discord-promo', label: 'Promo', icon: Gift, keywords: ['promo', 'promotion'] },
];
...
export default function DiscordMarketplace({ products }: DiscordMarketplaceProps) {
  ...
  const filteredProducts = useMemo(() => {
    return discordProducts.filter(p => {
      const title = p.title.toLowerCase();
      const cat = p.category.toLowerCase();
      ...
      if (activeCategory === 'all') {
        ...
        if (selectedCat && selectedCat.keywords) {
          categoryMatch = selectedCat.keywords.some(kw => title.includes(kw) || cat.includes(kw));
        }
      }
      ...
      if (searchQuery.trim() !== '') {
        ...
        searchMatch = title.includes(query) || cat.includes(query);
      }

      return categoryMatch && searchMatch;
    });
  }, [discordProducts, activeCategory, searchQuery]);

  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = { all: discordProducts.length };
    
    CATEGORIES.forEach(cat => {
      if (cat.id === 'all') return;
      let count = 0;
      if (cat.keywords) {
        count = discordProducts.filter(p => {
          const title = p.title.toLowerCase();
          const category = p.category.toLowerCase();
          return cat.keywords!.some(kw => title.includes(kw) || category.includes(kw));
        }).length;
      }
      counts[cat.id] = count;
    });
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-neutral-200 font-sans">
      ...
      <div 
        ...
        style={{ backgroundImage: "url('https://pub-bc67e2c069bd4bbeac1a9624c01f79db.r2.dev/uploads/ChatGPT%20Image%205%20%D8%AC%D9%88%D9%84%D8%A7%D8%A6%DB%8C%D8%8C%202026%D8%8C%2002_28_29%20AM.png')" }}
      >
        ...
        <div className="bg-[#1c1c1e] rounded-xl shadow-2xl p-6 sm:p-8 mb-8 border border-neutral-800">
          <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
            ...
            <div className="w-24 h-24 rounded-full bg-[#0a0a0b] flex items-center justify-center border-4 border-[#1c1c1e] shadow-inner flex-shrink-0">
               <img src="https://img.icons8.com/color/96/discord-logo.png" alt="Discord" className="w-16 h-16 object-contain" />
            </div>
            
            {/* Content */}
            <div className="flex-1 text-center sm:text-left">
              <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
                <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight m-0">Discord Marketplace</h1>
              ...
              <p className="text-sm text-neutral-400 max-w-2xl leading-relaxed">
                Discover premium Discord accounts, Nitro subscriptions, server boosts, and exclusive decorations. All your Discord needs in one place with instant automated delivery.
              </p>
            </div>
          </div>
            ...
            return (
              ...
              >
                <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 ${
                  ...
                  <category.icon className={`w-6 h-6 ${isActive ? 'text-black' : 'text-neutral-400'}`} />
                </div>
              ...
              >
                ...
                <div className="flex justify-between items-start gap-4 mb-4 flex-1">
                  ...
                  <div className="w-14 h-14 rounded-md bg-[#0a0a0b] border border-neutral-800 flex items-center justify-center p-1.5 flex-shrink-0 overflow-hidden">
                    <img 
                      src={product.media_url || 'https://img.icons8.com/color/96/discord-logo.png'} 
                      alt={product.title}
